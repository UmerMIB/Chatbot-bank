    ...
    switch (req.body.queryResult.action) {
        case 'bookHotel':
            db.collection('orders').add(params)
                .then(() => {
                    return res.send({
                        fulfillmentText: `${params.name} your hotel booking request for ${params.roomType} room is forwarded 
        ...
        case 'feedBack':
            db.collection('feedBack').add(params)
                .then(() => {
                    return res.send({
                        fulfillmentText: `Your ${params.feedBack} has been recorded. We will try to make better
